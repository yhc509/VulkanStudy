...
private:
    ...
    void drawFrame() {
        ...
        if (result == VK_ERROR_OUT_OF_DATE_KHR) {
            // 스왑체인이 surface와 호환되지 않아 사용할 수 없으면 재생성한다.
            recreateSwapChain();
            return;
        } else if (result != VK_SUCCESS && result != VK_SUBOPTIMAL_KHR) {
            // 스왑체인 속성이 더 이상 일치하지 않는 경우.
            throw std::runtime_error("failed to acquire swap chain image!");
        }

        updateUniformBuffer(currentFrame);

        // 교착상태 방지를 위해 제출되면 리셋해주기.
        ...
        if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR || framebufferResized) {
            // 이미지를 이미 획득했지만 스왑체인이 오래되어 최적이 아닌 경우.
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        ubo.proj[1][1] *= -1;       // OpenGL 용이라 y좌표가 반전되어야 함.
    ...
    */
    ...
    void createGraphicsPipeline() {
        ...
        */
        ...
        // 입력 어셈블리
        // 정점들을 어떤 지오매트리로 그릴 것인가? 프리미티브 재시작이 활성화 되는가?
        ...
        // 동적 상태
        // 파이프라인을 다시 만들지 않고 데이터를 지정할 수 있도록 함.
        // 뷰포트와 시저 사각형을 사용함.
